...
export default function SEOCountyPage() {
  ...
  const countyName = slug.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
  const [pages, setPages] = useState<SEOPageItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${API_BASE}/seo/pages?county=${countyName}&limit=200`)
      .then(r => r.json())
  ...
  return (
    <section className="py-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-zinc-900 mb-2">{countyName}</h1>
        <p className="text-zinc-600 mb-8">{pages.length} service pages</p>
          ...
          {pages.map((page) => (
            ...
            >
              ...
              <div className="flex gap-2 mt-3">
                <span className="text-xs bg-amber-50 px-2 py-1 rounded-full text-amber-700">{formatService(page.service_type)}</span>
                {page.industry && (
                  <span className="text-xs bg-zinc-100 px-2 py-1 rounded-full text-zinc-600">{page.industry}</span>
                )}
              </div>
            </Link>
          ))}
